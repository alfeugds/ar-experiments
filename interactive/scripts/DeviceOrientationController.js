...
var DeviceOrientationController = function (object, domElement) {
	...
	this.updateDeviceMove = function () {
		...
		return function () {
			...
			this.adjustAlphaOrientation()

			alpha = THREE.Math.degToRad((this.deviceOrientation.alpha && this.deviceOrientation.alpha + this.correctionAlpha) || 0); // Z
	...
	this.correctionAlpha = null;

	this.adjustAlphaOrientation = function(){
		if(!this.initialAlpha){
			this.initialAlpha = this.deviceOrientation.alpha
			this.correctionAlpha = -this.deviceOrientation.alpha
			return
		}
	}
